...
- 기능 목록 작성 및 README.md 업데이트
  - 기능 목록을 정리하고 이를 README.md 파일에 추가.
- 기능 구현
  1. 컴퓨터가 1에서 9까지 서로 다른 임의의 수 3개를 선택한다.
  2. 사용자가 3자리 숫자를 입력하면 검증을 수행한다.
  3. 입력된 숫자에 대해 스트라이크, 볼, 낫싱 결과를 반환한다.
  4. 사용자가 정답을 맞출 때까지 게임을 반복한다.
  5. 사용자가 정답을 맞추면 게임을 종료하고 재시작 버튼을 제공한다.
  6. 재시작 버튼을 클릭하면 새로운 게임을 시작한다.
  7. 입력이 올바르지 않은 경우 알림 메시지를 출력하고 재입력 받는다.
